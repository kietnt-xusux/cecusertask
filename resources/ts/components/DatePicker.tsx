...
export const DatePicker = (props: {
    ...
    const setPosition = () => {
        ...
        switch (props.position) {
            case "right":
                ...
                break;
            case "top":
                classText = props.small ? 'bottom-8' : "bottom-11"
                break;
            default:
                classText = "";
        }

        return "bg-white mt-1 rounded-lg shadow p-4 absolute w-72 z-30 " + classText;
    }
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Escape' || e.key === 'Enter') setShowDatePicker(false)
    }
    ...
    return (
        <div className={`relative ${props.className ?? ''}`} ref={wrapperRef}>
            <div className="cursor-pointer border rounded-md bg-white">
                <input type="text"
                    className={`w-full ${props.small ? 'px-1 py-0.5' : 'px-3 py-2'} rounded-md `
                        + `focus:outline-none focus:shadow-outline text-gray-600 font-medium cursor-pointer`}
                ...
                {
                    props.clearIcon && props.date !== '' &&
                    <div className="absolute top-0 right-0 px-3 py-2 h-full border-b border-t border-transparent" onClick={clearDate}>
